/* =============================================================
   MATERIALS INFORMATION PLATFORM — element-page.js  (v2)

   Shared renderer for element detail pages.
   Now located at root; element HTML files live in elements/
   so all paths back to root use ../ prefix.

   Each element page calls:   renderElementPage(ATOMIC_NUM);
   ============================================================= */
...
/*
  FEATURES — available interactive pages per element.
  Paths are relative to the elements/ subfolder (use ../ for root files,
  ../clouds/ for electron cloud files).
 ...
 * Called from individual element stub pages inside the elements/ subfolder.
 * All internal links use ../ to navigate back to the project root.
...
function renderElementPage(atomicNum) {
  ...
  if (!el) {
    document.body.innerHTML = '<p style="padding:40px;font-family:sans-serif">Element #' + atomicNum + ' not found.</p>';
    return;
  }
  ...
  /* HEADER — back button uses ../ because this file is called from elements/ subfolder */
  ...
  if (feats.electronCloud) {
    html += ab(feats.electronCloud, '&#9883;', 'Electron Cloud Animation', 'See quantum probability clouds for all electron orbitals', false);
  } else {
    html += ab(null, '&#9883;', 'Electron Cloud Animation', 'Interactive orbital visualisation &mdash; coming soon', true);
  }
  ...
  /* Industrial Uses and Discovery History: both active for all 118 elements */
  /* Links use ../ prefix because this file is called from the elements/ subfolder */
  html += ab('../industrial-uses.html?n=' + atomicNum, '&#127981;', 'Industrial Uses', 'Top industries &amp; companies that use this element', false);
  html += ab('../discovery-history.html?n=' + atomicNum, '&#128218;', 'Discovery History', 'Who discovered it, when, where, and the story behind the name', false);
  ...
  /* Write the completed HTML string into the page root container */
...
function pc(label, value) {
  return '<div class="prop-card"><div class="prop-label">' + label + '</div><div class="prop-value">' + value + '</div></div>';
}
...
function ab(href, icon, title, desc, disabled) {
  if (!disabled && href) {
    return '<a href="' + href + '" class="action-btn available"><span class="action-icon">' + icon + '</span><span class="action-title">' + title + '</span><span class="action-desc">' + desc + '</span></a>';
  }
  return '<div class="action-btn disabled"><span class="action-icon">' + icon + '</span><span class="action-title">' + title + '</span><span class="action-desc">' + desc + '</span></div>';
}
